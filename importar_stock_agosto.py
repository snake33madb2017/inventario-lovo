...
# Ignorar advertencias de pandas sobre openpyxl (Default style missing)
warnings.filterwarnings('ignore', category=UserWarning, module='openpyxl')

EXCEL_FILE = "stock agosto actualizado terminado.xlsx"
...
def split_bottles_and_pct(cantidad):
    ...
    return botellas_llenas, f"{pct_val}%" if pct_val > 0 else "-"
...
def parse_stock_lovo(excel_path, alias_dict):
    try:
        ...
        print(f"Error reading 'Stock lovo': {e}")
        return []
    records = []
...
def parse_cristaleria(excel_path, alias_dict):
    try:
        ...
        print(f"Error reading 'Cristaleria': {e}")
        return []
    records = []
...
def parse_stock_producciones(excel_path, alias_dict):
    try:
        ...
        print(f"Error reading 'Stock producciones': {e}")
        return []
    records = []
    for _, row in df.iterrows():
...
def run_import():
    ...
    # Create table if not exists (although it should already exist based on server.py)
    cursor.execute("""
    ...
    # We use the correct table 'registros' and date '31/08/2026'
    ...
    # Optional: Delete existing records for that date to avoid duplicates?
    # cursor.execute("DELETE FROM registros WHERE fecha = '31/08/2026'")
    ...
    batch_data = [
        ...
        for r in all_records if r["cantidad_dictada"] > 0
    ]
    cursor.executemany(insert_query, batch_data)
    conn.commit()
    conn.close()
    print("✅ ¡Importación completada con éxito en SQLite (tabla registros)!")
